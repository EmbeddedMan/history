...
/*
	...
	Returns DFS_ERRMISC for any error.
...
{
	uint32_t i, result = DFS_ERRMISC, scratchcache = 0;
	...
	for (i=unusedfat; i < volinfo->numclusters; i++) {
	...
	while (remain && result == DFS_OK) {
		...
		// Case 2 - File pointer is on sector boundary
			...
			if (remain >= SECTOR_SIZE) {
			    // optimization!
			    if (OPTIMIZE && div(fileinfo->pointer, fileinfo->volinfo->secperclus*SECTOR_SIZE).rem+len <= fileinfo->volinfo->secperclus*SECTOR_SIZE) {
			        n = remain/SECTOR_SIZE;
			        if (n > fileinfo->volinfo->secperclus - div(fileinfo->pointer, fileinfo->volinfo->secperclus*SECTOR_SIZE).rem/SECTOR_SIZE) {
			            n = fileinfo->volinfo->secperclus - div(fileinfo->pointer, fileinfo->volinfo->secperclus*SECTOR_SIZE).rem/SECTOR_SIZE;
			        }
			    } else {
			        n = 1;
			    }
			    
				result = DFS_ReadSector(fileinfo->volinfo->unit, buffer, sector, n);
				remain -= n*SECTOR_SIZE;
				buffer += n*SECTOR_SIZE;
				fileinfo->pointer += n*SECTOR_SIZE;
				bytesread = n*SECTOR_SIZE;
			}
...
/*
	Rename a file
	scratch must point to a sector-sized buffer
	newpath and path must have identical directory components
*/
uint32_t DFS_RenameFile(PVOLINFO volinfo, uint8_t *path, uint8_t *newpath, uint8_t *scratch)
{
    uint32_t rv;
    char *p, *q;
	PDIRENT de = (PDIRENT) scratch;
	...
	// unlink the destination name if it exists
	rv = DFS_UnlinkFile(volinfo, newpath, scratch);
	if (rv && rv != DFS_NOTFOUND) {
	    return rv;
	}
	
	// convert the destination name to dir entry format
	p = (char *)newpath;
	for (;;) {
	    q = strchr(p, '/');
	    if (! q) {
	...
	DFS_CanonicalToDir(filename, (uint8_t *)p);
	...
	while (remain && result == DFS_OK) {
		...
		// Case 2 - File pointer is on sector boundary
			...
			if (remain >= SECTOR_SIZE) {
				...
				result = DFS_WriteSector(fileinfo->volinfo->unit, buffer, sector, n);
				remain -= n*SECTOR_SIZE;
				buffer += n*SECTOR_SIZE;
				fileinfo->pointer += n*SECTOR_SIZE;
				if (fileinfo->filelen < fileinfo->pointer) {
